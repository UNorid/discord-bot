...
HERO_ALIASES = {
    "брист": "Bristleback","сф": "Shadow Fiend", "ам": "Anti-Mage", "па": "Phantom Assassin", "мортред": "Phantom Assassin",
...
# Описание команд для меню !команды: (описание, категория)
COMMAND_INFO = {
    "профиль":   ("Показать твой уровень, звание и опыт.", "⭐ Игровые и общие команды"),
    "топ":       ("Таблица топ-10 игроков сервера.", "⭐ Игровые и общие команды"),
    "ежедневка": ("Забрать ежедневную награду (XP).", "⭐ Игровые и общие команды"),
    "казино":    ("Сыграть в кости на XP против бота (ставка от 10 до 500).", "⭐ Игровые и общие команды"),
    "дуэль":     ("Устроить уличный замес на XP с другим бойцом.", "⭐ Игровые и общие команды"),
    "команды":   ("Вызвать это справочное меню в ЛС.", "⭐ Игровые и общие команды"),

    "матч":      ("Анализ матча Dota 2 через OpenDota API.", "🐲 Dota 2"),
    "герой":     ("Разбор героя: атрибуты, роли, предметы, матчапы.", "🐲 Dota 2"),
    ...
    "колесо":    ("Запустить Колесо Фортуны на розыгрыш временной роли.", "🛠️ Интерфейс"),

    "очистить":  ("Удалить сообщения в канале.", "🛡️ Модерация и администрирование"),
    "мут":       ("Отправить нарушителя в таймаут.", "🛡️ Модерация и администрирование"),
    "кик":       ("Выгнать игрока с сервера.", "🛡️ Модерация и администрирование"),
    "настройка": ("Полностью создать структуру каналов и ролей с нуля.", "🛡️ Модерация и администрирование"),
    "секрет":    ("Открыть доступ к скрытому бункеру.", "🛡️ Модерация и администрирование"),
}

CATEGORY_ORDER = [
    "⭐ Игровые и общие команды",
    "🐲 Dota 2",
    "🛠️ Интерфейс",
...
async def commands_list(ctx):
    """Показывает список команд, доступных именно этому пользователю
    (зависит от его роли/прав — проверяется через реальные декораторы команд)."""
    categories = {}

    for name, (desc, category) in COMMAND_INFO.items():
        cmd = bot.get_command(name)
        if cmd is None or cmd.hidden:
            continue

        # Проверяем реальные условия команды (роли/права и т.д. из декораторов)
        try:
        ...
        line = f"`!{cmd.name}`{aliases} — {desc}"
        categories.setdefault(category, []).append(line)

    embed = discord.Embed(
        title="📜 СПИСОК ДОСТУПНЫХ ТЕБЕ КОМАНД",
        description="Вот команды, которыми ты можешь пользоваться прямо сейчас. Не забывай про префикс `!`",
        color=0x8B0000,
    )

    if not categories:
        embed.add_field(name="…", value="Нет доступных команд.", inline=False)
    else:
    ...
    try:
        ...
        await ctx.send(
            f"📬 {ctx.author.mention}, список доступных тебе команд отправлен в личные сообщения!",
            delete_after=6,
        )
    except discord.Forbidden:
        await ctx.send(
            f"⚠️ {ctx.author.mention}, у тебя закрыты личные сообщения! Открой их, либо смотри сюда:",
            embed=embed,
        )
...
async def hero_command(ctx, *, hero_query: str):
    async with ctx.typing():
        try:
            ...
            # Используем функцию поиска по русским прозвищам и нечетким совпадениям
...
async def start_web_server():
    ...
    try:
        ...
        print(f"Порт {port} уже занят, пропускаем запуск веб-сервера...")
